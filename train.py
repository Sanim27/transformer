...
def greedy_decode(model,source,source_mask,tokenizer_src,tokenizer_tgt,max_len,device):
    ...
    while True:
        ...
        #select the token with the max prob because it is greedy search
...
def run_validation(model,validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_msg,global_stare,writer,num_examples=2):
    model.eval()
    count=0
    console_width=80
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            #print to console using tqdm method
...
#             decoder_mask=batch['causal_mask'].to(device) # ( batch , 1 , seq_len ,seq_len)
